...
public class touPiao_ZU_xuanx {
	...
	public static void main(String[] args) throws Exception {
		...
		read1.getExcel("D:\\Data\\toupiao_H.xls", "投票组内容1");
		for(int i=1;i<read1.rows;i++){
			...
			for(int n=2;n<=Row.size();n++){
				if(driver.findElement(By.xpath("//*[@id='data']/table/tbody/tr["+n+"]/td[2]")).getText().trim().equals(read1.readColumn("组名", i))){
			...
			GroupID.selectByVisibleText(read1.readColumn("组名", i));
			Thread.sleep(1000);
			...
			read2.getExcel("D:\\Data\\toupiao_H.xls", "组选项");
			for(int j=1;j<read2.rows;j++){
				if(read2.readColumn("标题", j).equals(read1.readColumn("投票主题", i))){
				for(int n=1;n<read2.columns;n++){
					if(read2.readColumn("选项"+n, j).equals("")) break;
					driver.findElement(By.id("txtItem"+(n-1))).sendKeys(read2.readColumn("选项"+n,j));
					int m=0;
					if(m<n){
						m++;
			...
			Thread.sleep(1000);
			mod.repeatModule(driver, "投票管理", "投票组列表");
		}	
	}
}
