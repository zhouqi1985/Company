...
public class PayMUTCard {
	...
	public static void main(String[] args) throws Exception {
		...
		read.getExcel("D:\\Data\\pay.xls", "MUTCard");
		driver.get("https://pay.zygames.com/");
		Thread.sleep(500);
		for(int i=1;i<read.rows;i++){
			count++;
			PayModule mod =new PayModule();
			mod.getModule(driver, read.readColumn("充值卡类型", i));
			...
			driver.findElement(By.id("Sn")).sendKeys(read.readColumn("充值卡序列号", i));
			driver.findElement(By.id("Password")).sendKeys(read.readColumn("充值卡密码", i));
			driver.findElement(By.id("CardMoney")).sendKeys(read.readColumn("充值卡面额", i));
			...
			if(judge.equals("y")){
				System.out.println(count+"."+read.readColumn("充值卡类型", i)+"-"+read.readColumn("金额", i)+"元 跳转成功");
			}else {
				System.out.println(count+"."+read.readColumn("充值卡类型", i)+"-"+read.readColumn("金额", i)+"元 跳转失败");
			}
			Thread.sleep(500);
		}
