...
public class PayAllBank {
	...
	public static void main(String[] args) throws Exception {
		...
		read.getExcel("D:\\Data\\pay.xls", "AllBank");
		driver.get("https://pay.zygames.com/");
		Thread.sleep(500);
		for(int i=1;i<read.rows;i++){
			count++;
			if(read.readColumn("银行", i).equals("")) break;
			PayModule module =new PayModule();
			module.getModule(driver, "银行卡充值");
			driver.findElement(By.cssSelector("input[text=\""+read.readColumn("银行", i)+"\"]")).click();
			...
			if(judge.equals("y")){
				System.out.println(count+"."+read.readColumn("银行", i)+"-"+read.readColumn("金额", i)+"元 跳转成功");
			}else {
				System.out.println(count+"."+read.readColumn("银行", i)+"-"+read.readColumn("金额", i)+"元 跳转失败");
			}
			Thread.sleep(500);
		}
