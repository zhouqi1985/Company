...
public class PayMoWeb {
	...
	public static void main(String[] args) throws Exception {
		...
		for(int i=1;i<read.rows;i++){
			...
			driver.findElement(By.id("Mobile")).sendKeys(read.readColumn("手机号", i));
			driver.findElement(By.id("ConfirmMobile")).sendKeys(read.readColumn("确认手机号", i));
			...
			if(judge.equals("y")){
				System.out.println(count+".移动短信充值-"+read.readColumn("金额", i)+"元 跳转成功");
			}else {
				System.out.println(count+".移动短信充值-"+read.readColumn("金额", i)+"元 跳转失败");
			}
			Thread.sleep(500);
		}
	}
