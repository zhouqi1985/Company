...
public class hongbao_ptkl {
	...
	public static void main(String[] args) throws Exception {
		...
		Thread.sleep(500);
		driver.findElement(By.cssSelector("input[value=\"神秘商店概率\"]")).click();
		readExcel read =new readExcel();
		read.getExcel("E:\\Data\\guaka.xls", "shenmi");
		for(int i=1;i<read.rows;i++){
			count++;
			if(read.readColumn("礼包", i).equals("")) break;
			Thread.sleep(500);
			...
			Thread.sleep(500);
			System.out.println(count+"."+read.readColumn("类型", i)+"-"+read.readColumn("礼包", i)+"礼包是否在概率表中添加成功："+driver.findElement(By.className("red")).getText().trim().equals("Success"));
